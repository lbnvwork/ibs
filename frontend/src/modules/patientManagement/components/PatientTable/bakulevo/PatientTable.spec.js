...
describe('PatientTable.vue', () => {
  ...
  describe('3.58: триаж-бейджи, заголовок и карточка (СЦ-3.58.15)', () => {
    const p = (overrides = {}) => ({
      id: 1,
      name: 'Иванов Пётр',
      age: '45',
    ...
    it('triageBadge: highlightRed → danger «Угроза кровотечения»', () => {
      const wrapper = mountTable()
      expect(wrapper.vm.triageBadge(p({ highlightRed: true }))).toEqual({
        label: 'Угроза кровотечения',
        type: 'danger',
      })
    })

    it('triageBadge: highlightBlue → info «Риск тромбоза»', () => {
      const wrapper = mountTable()
      expect(wrapper.vm.triageBadge(p({ highlightBlue: true }))).toEqual({
        label: 'Риск тромбоза',
        type: 'info',
      })
    })

    it('triageBadge: без флагов → success «В норме»', () => {
      const wrapper = mountTable()
      expect(wrapper.vm.triageBadge(p())).toEqual({ label: 'В норме', type: 'success' })
    })

    it('заголовок «Пациенты» + счётчик из totalCount', () => {
      const wrapper = mountTable({ patients: [p()], totalCount: 42 })
      expect(wrapper.find('.table-title h3').text()).toBe('Пациенты')
      expect(wrapper.find('.patient-count').text()).toBe('42')
    })

    it('счётчик падает на patients.length при totalCount = 0', () => {
      const wrapper = mountTable({ patients: [p(), p({ id: 2 })], totalCount: 0 })
      expect(wrapper.find('.patient-count').text()).toBe('2')
    })

    it('карточка-контейнер: корень имеет класс patient-card', () => {
      const wrapper = mountTable()
      expect(wrapper.find('.patient-table').classes()).toContain('patient-card')
    })

    it('бейдж рендерится в default-строке по highlightRed', () => {
