...
describe('VitalsCard.vue', () => {
  ...
  describe('3.58: плитки показателей (СЦ-3.58.16)', () => {
    it('рендерит показатели плитками (.vital-tile)', async () => {
      const wrapper = mountVitalsCard({}, [{ hb: 140, heartRate: 70, systolicPressure: 120, diastolicPressure: 80, saturation: 98, weight: 80 }])
      await flushPromises()

      expect(wrapper.find('.vitals-tiles').exists()).toBe(true)
      expect(wrapper.findAll('.vital-tile')).toHaveLength(5)
      expect(wrapper.text()).toContain('Гемоглобин')
    })
  })
})
